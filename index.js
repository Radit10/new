...
//punya gw, File Sticker
heker1 = fs.readFileSync("./storage/sticker/heker1.png")
heker2 = fs.readFileSync("./storage/sticker/heker2.png")
heker3 = fs.readFileSync("./storage/sticker/heker3.png")
heker4 = fs.readFileSync("./storage/sticker/heker4.png")
var HekerList = [heker1,heker2,heker3,heker4]
 	...
 	var budy = (typeof m.text == 'string' ? m.text : '')
       var prefix = prefa ? /^[°•π÷×¶∆£¢€¥®™+✓_=|~!?@#$%^&.©^]/gi.test(body) ? body.match(/^[°•π÷×¶∆£¢€¥®™+✓_=|~!?@#$%^&.©^]/gi)[0] : "" : prefa ?? global.prefix
		...
		try {
			...
			let chats = db.data.chats[m.chat] //punya gw
...
//━━━━━━━━━━━━━━━━━━━━━━[ Voice ]━━━━━━━━━━━━━━━━━━━━━━━━━━//punya gw							
        ...
        } else if (budy.match(/(([sa])?yan?([gk])|c([oe])?([gc])?an)/gi)) {
           alpha.sendPresenceUpdate('recording', m.chat)
        ...
        } else if (budy.match(/(ad?m([ie])n|ow?ner|pe?mi?lik|bu?at|ba?ca|ba?co([td])|ad?u|la?wan|ge?lu([td])|sp?am|([vp])i?([rl])te?([xk])|la?wa?k|([vp])i?rus|l([ae])?g)/gi)) {
           alpha.sendPresenceUpdate('recording', m.chat)
           await sendAudio("mp3/kalo_mau_bacot.mp3")
        } else if (budy.match(/(kam?pung|ru?mah|tem?pat|ti?ng?gal|ra?wa|hac?ker|he?ng?ker|mis?kin|wk?wk|nga?kak|be?ngek|aw?ok|Ao?wk|mu?ka|gan?teng|s([au])m?pah)/gi)) {
           alpha.sendPresenceUpdate('recording', m.chat)
           await sendAudio("mp3/toni_kampung_rawa.mp3")
        }
    }  
...
if (!m.isGroup) return reply(lang.groupOnly())
        ...
        reply(lang.OffBef('Nsfw'))
			...
			break
			case 'antilink': case 'al': //punya gw
			if (!m.isGroup) return reply(lang.groupOnly())
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'block': { //punya gw
                ...
                await alpha.updateBlockStatus(users, 'block').then((res) => reply('Sukses Memblokir')).catch((err) => m.reply(jsonformat(err)))
            }
            break
            case 'unblock': { //punya gw
                if (!m.key.fromMe && !isCreator) return reply(lang.ownerOnly())
                let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
                await alpha.updateBlockStatus(users, 'unblock').then((res) => reply('Sukses Unblokir')).catch((err) => m.reply(jsonformat(err)))
            }
            break
            case 'rules':{
...
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[ CASE ]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━//punya gw        
      ...
      if (!m.key.fromMe && !isCreator) return
        ...
        anu = args.join(' ').split('|')
        siji = anu[0] !== '' ? anu[0] : "https://chat.whatsapp.com/BZqRmA7cN576QJdZhnBpOM"
        loro = typeof anu[1] !== 'undefined' ? anu[1] : "🤤 BOKEP ⭕ SD ⭕ SMP ⭕ SMA 💦"
        teks2 = `Syarat masuk grup

1.Bagikan tautan ini ke 3 grup
2.Bagikan dulu baru bisa masuk
3.Klau gak di bagikan otomatis gak bisa masuk

*KHUSUS BOKEP ANAK SEKOLAH SMP/SMA VIRAL*

${siji}

*SELAMAT BERGABUNG*

gak share gak bisa masuk🙏`
        //fake = await getBuffer('https://telegra.ph/file/70ef2a2fdb2c97aa4e877.jpg') //kontol
        //fake = await getBuffer('https://telegra.ph/file/57a4944362d42245ea3f9.jpg') //lonthe memek
        //fake = await getBuffer('https://telegra.ph/file/bd90c29811fa1db8fd150.jpg') //pap tt
        //fake = await getBuffer('https://telegra.ph/file/956e2b15dfdb6f9b71a7d.png') //nay lonthe

        //fake = await getBuffer('https://telegra.ph/file/75a5b99b4f9a39b393b47.png') //bts gay
        //fake = await getBuffer('https://telegra.ph/file/66ca4a792d63c6f19f389.jpg') //tuhan bts
        fake = await getBuffer('https://telegra.ph/file/77a11d02e99ca2c276d2c.png') //BTS tuhan ku
        ...
        break
            ...
            case 'on':
                ...
                if (args[0] === 'autoread') {
                    ...
                    reply('unavailable on')
                } else if (args[0] === 'pause') {
                    global.pause = true
                    reply('pause on')
                } else if (args[0] === 'autovoice') {
                    global.autovoice = true
                    reply('autovoice on')
                } else if (args[0] === 'autobio') {
                    global.autobio = true
                    reply('autobio on')
                }            
            break
            ...
            case 'off':
                ...
                if (args[0] === 'autoread') {
                    ...
                    reply('unavailable off')
                } else if (args[0] === 'pause') {
                    global.pause = false
                    reply('pause off')
                } else if (args[0] === 'autovoice') {
                    global.autovoice = false
                    reply('autovoice off')
                } else if (args[0] === 'autobio') {
			...
			case 'antivirus': case 'antivirtex': case 'av':
			if (!m.isGroup) return reply(lang.groupOnly())
			if (!isGroupAdmins && !isGroupOwner && !isCreator) return reply(lang.adminOnly())
			if (!isBotAdmins) return reply(lang.botNotAdmin())
			if (args[0] === "on") {
...
console.log(err)
await m.reply(util.format(err)) //punya gw
}
}
